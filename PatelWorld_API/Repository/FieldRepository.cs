...
namespace PatelWorld_API.Repository
{
    public class FieldRepository : IFieldRepository
    {
        ...
        public TblFields GetSingleField(CommonEditdelete obj)
        {
            ...
            {
                DBHelper db = new DBHelper(_configuration);
                DataTable dt = db.ExecuteQuery("SELECT TOP 1 * FROM TblFields WITH (NOLOCK) WHERE Id = '"+obj.Id+"'  AND IsActive = 1 AND IsDelete != 1 ");
        ...
        public CommonResponseModel DeleteField(CommonEditdelete obj)
        {
            ...
            {
                DBHelper db = new DBHelper(_configuration);
                DataTable dt = db.ExecuteQuery("UPDATE TblFields  WITH (ROWLOCK) SET IsActive = 0, IsDelete = 1 WHERE Id = '"+ obj.Id + "'  AND IsActive = 1 AND IsDelete != 1");
        ...
        public CommonResponseModel CreateUpdateByMapping(TblFields field)
        {
            ...
            try
            {
                if (field.Id > 0)
                {
                    var existsQuery = "SELECT COUNT(1) FROM TblFields WHERE Id != '"+ field.Id + "' AND LOWER(Name) = '"+ field.Name + "' AND IsActive = 1 AND IsDelete != 1";
                ...
                {
                    var existsQuery = "SELECT COUNT(1) FROM TblFields WHERE LOWER(Name) = '"+ field.Name + "' AND IsActive = 1 AND IsDelete != 1";
